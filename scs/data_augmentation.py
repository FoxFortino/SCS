...
def augment(
    ...
    for sn_type, num_augment in zip(sn_types, num_augments):
        ...
        sn_type_df_rep = sn_type_df.iloc[
            np.tile(np.arange(sn_type_df.shape[0]), num_augment)
        ].copy(deep=True)
        ...
        masked_spikes = gen_spikes(
            masked_fluxes, masked_wvl0, spike_scale, max_spikes, rng
        )
...
def gen_spikes(spectrum, wvl, spike_scale, max_spikes, rng):
    num_spikes = stats.randint.rvs(low=0, high=max_spikes, size=1, random_state=rng)[0]

    spike_wvls = stats.uniform.rvs(
        loc=wvl[0], scale=wvl[-1] - wvl[0], size=num_spikes, random_state=rng
    )

    spike_inds = np.digitize(spike_wvls, wvl)

    spike_dir = stats.binom.rvs(n=1, p=0.50, size=num_spikes, random_state=rng)
